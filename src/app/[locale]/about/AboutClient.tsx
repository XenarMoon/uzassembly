...
const resolveLocalized = (locale: string, uz?: string, ru?: string, en?: string) => {
...
const chairmanConfig = {
  ...
  image: '/images/team/Photo-58.jpg',
}
...
export default function AboutClient() {
  ...
  const teamMembers = useMemo<TeamMember[]>(() => {
    if (dynamicTeam.length) {
      return dynamicTeam.map((member, index) => {
        const name = resolveLocalized(locale, member.nameUz, member.nameRu, member.nameEn)
  ...
  const otherMembers = useMemo(
    () => teamMembers.filter((member) => !member.featured && member.category === 'member'),
    [teamMembers]
  )

  const advisorMembers = useMemo(
    () => teamMembers.filter((member) => member.category === 'advisor'),
    [teamMembers]
  )
