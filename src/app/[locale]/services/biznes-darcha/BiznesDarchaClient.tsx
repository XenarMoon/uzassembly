...
const servicesConfig = [
  ...
  { key: 'licensing', icon: Shield },
]

const serviceCategoriesConfig = [
  { key: 'registration', icon: Building2, color: 'gold' },
  { key: 'tax', icon: Receipt, color: 'emerald' },
  { key: 'hr', icon: ClipboardList, color: 'turquoise' },
]

const benefitsConfig = [
  { key: 'fast', icon: Clock },
  { key: 'manager', icon: HeadphonesIcon },
  { key: 'guarantee', icon: FileCheck },
  { key: 'government', icon: Landmark },
]

export default function BiznesDarchaClient() {
  const t = useTranslations('biznesDarchaPage')
  ...
  const serviceCategories = serviceCategoriesConfig.map((category) => ({
    ...category,
    title: t(`categories.${category.key}.title`),
    description: t(`categories.${category.key}.description`),
    items: t.raw(`categories.${category.key}.items`) as string[],
  }))
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
          ...
          >
            ...
            <h2 className="font-heading text-2xl lg:text-3xl font-semibold text-white mb-4">
              {t('categoriesSection.headline')}{' '}
              <span className="text-gradient-gold font-display">{t('categoriesSection.headlineHighlight')}</span>
            </h2>
          </motion.div>
          ...
          >
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {benefits.map((benefit) => (
                <div key={benefit.key} className="text-center p-4">
